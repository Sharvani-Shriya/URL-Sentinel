...
|---|---|---|---|
| 21 Lexical Features | 89.85% | N/A | Random Forest |
...
Formally, for an input URL $u$ and its extracted feature vector $\mathbf{f}(u)$, the text serialization function $\mathcal{T}(u)$ maps the URL attributes into a multi-sentence narrative document. To demonstrate this transformation, consider an example regional phishing URL $u_{\text{example}}$ attempting to impersonate a Mongolian government portal:

- **Example Input URL:**  
  `http://secure-login.gov.mn.verify-account.xyz/update.php`

- **Generated Serialized Document $\mathcal{T}(u_{\text{example}})$:**  
  > *"The URL is `http://secure-login.gov.mn.verify-account.xyz/update.php`. It has a length of 57 characters. Uses HTTPS: No. Uses raw IP address: No. Is shortener: No. Is official gov/edu: No. Suspicious TLD: Yes. Brand impersonation: Yes. Contains urgency words: Yes. Security keywords used: 2. Abnormal structure: Yes."*

Serializing URLs into natural-language narratives allows pre-trained Transformer language models to leverage their rich linguistic pre-training to map structural and behavioral invariants (e.g., non-HTTPS, IP-hosted URLs with brand impersonation keywords on suspicious TLDs) into a smooth, semantically meaningful metric manifold, grouping URLs by structural threat behavior rather than superficial string overlap.
...
**Figure 5.** Decision fusion pipeline combining the ML classifier's prior, the Forensic Reasoning Engine's weighted evidence score, and a similarity-gated RAG adjustment into a single calibrated verdict.
